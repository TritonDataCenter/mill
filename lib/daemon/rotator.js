...
function Rotator(opts, cb) {
    ...
    mod_assert.string(opts.mantaLogPath, 'opts.mantaLogPath');

    opts.log.info({opts: opts}, 'Uploader:new: entering');
    ...
    this._mantaLogPath = opts.mantaLogPath; /* manta log path */
    ...
    mod_vasync.pipeline({funcs: [
        ...
        function _upload(_, _cb) {
            upload(self, function (err) {
                if (err) {
                    err = new mod_verror.VError(err, 'unable to upload logs');
                }
...
function getMantaPath(self, fileName) {
    ...
    // /$user/stor/mill/logs/$service/$year/$month/$day/$hour/$instance:generic.log
    var path = mod_sprintf('%s/%s/%s/%s/%s/%s/%s-%s', self._mantaLogPath, self._service,
                           year, month, date, hour, self._instance, file);
    ...
    mod_vasync.pipeline({funcs: [
        function _listDir(_, _cb) {
            mod_fs.readdir(self._rotateDir, function (err, files) {
                if (err) {
                    err = new mod_verror.VError(err, 'unable to list rotate ' +
                                                'log dir');
                    return _cb(err);
                }
        ...
        function _upload(_, _cb) {
            ...
            _.files.forEach(function (file) {
                ...
                self._mantaClient.put(path,
                                      mod_fs.createReadStream(fullFilePath),
                                      {mkdirs: true},
                                      function (err) {
                    barrier.done('mput ' + file);
                    if (err) {
                        self._isUpload = false;
                        return cb(new mod_verror.VError(err,
                ...
                // make this better with retries
                //mod_mput({
                    //path: path,
                    //file: self._rotateDir + '/' + file,
                    //log: log,
                    //client: self._mantaClient
                //}, function (err) {
                //});
            });
        },
        ...
        if (err) {
            err = new mod_verror.VError(err, 'unable to upload all logs');
            log.error({err: err});
        }
...
new Rotator({
    ...
    logType: 'generic',
    mantaLogPath: '/yunong/stor/mill/logs'
}, function (err)  {
