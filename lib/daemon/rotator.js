...
function Rotator(opts) {
    ...
    mod_assert.string(opts.jobName, 'opts.jobName');
    ...
    this._jobName = opts.jobName; /* the name of the job*/
...
function submitJob(self, keys, cb) {
    ...
    log.info({jobName: self._jobName}, 'Rotator.submitJob: entering');

    mod_vasync.pipeline({funcs: [
        function _getJob(_, _cb) {
            m.listJobs({name: self._jobName}, function (err, res) {
                if (err) {
        ...
        function _createNewJob(_, _cb) {
            ...
            m.createJob(self.jobConfig, {name: self._jobName},
                        function (err, jobId) {
                if (err) {
